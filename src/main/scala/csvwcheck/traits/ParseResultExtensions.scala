...
object ParseResultExtensions {
  ...
  implicit class ParseResultExtensions[T1](parseResult: ParseResult[T1]) {
    /**
      ...
      * @return
      */
    def flatMapStartAccumulating[T2](mapToNextParseResult: T1 => ParseResult[T2]): ParseResult[(T1, T2)] = {
  ...
  implicit class TupleParseResultExtensions[T1](parseResult: ParseResult[T1]) {
    /**
      ...
      * @return
      */
    def flatMapKeepAccumulating[T2](mapToNextParseResult: T1 => ParseResult[T2])
                                   (implicit prepend: tuple.Prepend[T1, Tuple1[T2]]): ParseResult[prepend.Out] = {
      parseResult.flatMap(t1Value =>
        mapToNextParseResult(t1Value)
